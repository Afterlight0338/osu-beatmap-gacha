...
export const ChangelogPage: React.FC<ChangelogPageProps> = ({ onSelectBeatmap }) => {
  ...
  // Compute tier shifts dynamically from pool
  const allShifts = useMemo<TierShiftItem[]>(() => {
    if (!pool || pool.length === 0) return [];

    // Reconstruct linear baseline ranking (58% favs / 42% plays)
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-8 animate-fade-in pb-12">
      ...
      {/* Version Release Selector & Summary Card */}
        ...
        <div className="flex flex-wrap items-center gap-2">
          {CHANGELOG_VERSIONS.map((v) => (
            <button
              ...
              className={`px-4 py-2 rounded-xl text-xs font-mono font-bold transition-all border flex items-center space-x-2 ${
      ...
      {/* Interactive Map Shifts Explorer */}
      <div className="space-y-4">
            ...
            {paginatedShifts.map((s) => {
              ...
              return (
                <div
                  ...
                  className={`group relative rounded-2xl bg-slate-900/80 border p-3.5 transition-all duration-200 hover:scale-[1.01] hover:bg-slate-800/80 cursor-pointer shadow-lg space-y-3 ${
                    isMajorBuff
                ...
                >
                  ...
                  {/* Bottom: 4D Dimension Metric Chips */}
...
fs.writeFileSync("src/pages/ChangelogPage.tsx", content);
console.log("src/pages/ChangelogPage.tsx written successfully.");
